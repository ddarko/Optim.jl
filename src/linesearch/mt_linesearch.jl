...
#
#           abs(gradf(x + stp * s)' * s)) <= g_tol * abs(gradf(x)' * s).
#
#     If f_tol is less than g_tol and if, for example, the function
...
#
#        subroutine cvsrch(fcn,n,x,f,g,s,stp,f_tol,g_tol,x_tol,
...
#
#  f_tol and g_tol are nonnegative input variables. Termination
...
function mt_linesearch!{T}(fcn::Union{DifferentiableFunction,
   ...
   if n <= 0 || stp <= 0.0 || f_tol < 0.0 || g_tol < 0.0 ||
   ...
   while true
      ...
      if f <= ftest1 && abs(dg) <= -g_tol * dginit
         info = 1
      end
      ...
      if stage1 && f <= ftest1 && dg >= min(f_tol, g_tol) * dginit
         stage1 = false
      end
